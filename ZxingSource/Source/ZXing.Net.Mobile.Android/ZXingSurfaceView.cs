			...
			if (camera != null)
			{
				if (!tokenSource.IsCancellationRequested)
				{
					...
					try
					{
						...
						//List<string> focusModes =(List<string>) p.SupportedFocusModes;

						//if (focusModes != null && focusModes.Contains(Android.Hardware.Camera.Parameters.FocusModeAuto))
						//{
					...
					{
						Android.Util.Log.Info("AutoFocus Exception", "AUTOFOCUS EXCEPTION: " + e.ToString());
					}
				}
			}
        }
